...
//
// Unlike the A08 kernel (which OVERWROTE out via mm.mul + store), this LOADS the
// existing out tile into the accumulator first (mm.from_vector) and MACs over all
// r, so repeated calls across the c-loop accumulate -- this is the "explicit OUT
// load" the A08 feedback required.
extern "C" void matmul(bfloat16 const *__restrict in0,
  ...
  for (unsigned p = 0; p < 2; ++p)
    for (unsigned q = 0; q < 2; ++q) {
      MMUL mm;
      // Seed the accumulator with the current out tile (partial sum over c so far).
      mm.from_vector(aie::load_v<64>(out + p * 128 + q * 64));
